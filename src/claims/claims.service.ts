...
export class ClaimsService {
  ...
  async findAll(pagination: PaginationDto, filters: FilterClaimsDto) {
    ...
    const result = await paginate(
      ...
      {
        where,
        relations: ['employee', 'employee.company', 'benefitPackage'],
    ...
    return {
      items: toDtoArray(ClaimResponseDto, result.items),
      meta: result.meta,
    };
  }
  ...
  async findByCompany(companyId: number, filters: FilterClaimsDto) {
    const where: FindOptionsWhere<Claim> = {
  ...
  async getRemainingAmount(packageId: number, employeeId: number) {
    const pkg = await this.benefitPackagesRepository.findOneBy({
      id: packageId,
    });
